...
// @ts-ignore;
import { ArrowLeft, History, Bell, Star, ThumbsUp, MessageSquare, Flag, Bug, Lightbulb, Heart, TrendingUp, BarChart3, CheckCircle, Comments, Upload, ChevronLeft, ChevronRight, Save, PaperPlane, Filter, Clock, UserCheck, AlertCircle } from 'lucide-react';

export const UserFeedbackComplete = ({
  onBack,
  onMyReviews,
  onNotifications
}) => {
        ...
        <CardContent className="p-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <Button variant="ghost" size="sm" onClick={() => handleLike(review.id)} className="text-white/60 hover:text-white">
                <ThumbsUp className="w-4 h-4 mr-1" />
                有帮助 ({review.helpful})
              </Button>
              <Button variant="ghost" size="sm" onClick={() => handleReply(review.id)} className="text-white/60 hover:text-white">
                <MessageSquare className="w-4 h-4 mr-1" />
                回复 ({review.replies})
              </Button>
            </div>
            <Button variant="ghost" size="sm" onClick={() => handleReport(review.id)} className="text-white/60 hover:text-white">
  ...
  const renderFeedbackTypeCard = type => {
    ...
    return <Button key={type.value} variant="ghost" onClick={() => handleFeedbackTypeSelect(type.value)} className={`p-6 text-left justify-start h-auto ${feedbackType === type.value ? 'bg-white/20 border-2 border-blue-400' : 'bg-white/10 hover:bg-white/20'}`}>
  ...
  return <div className="space-y-8">
      ...
      {activeTab === 'reviews' && <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            {renderStatCard('本月新增评价', reviewStats.monthlyReviews, Comments, 'bg-blue-500')}
            {renderStatCard('平均回复时间', reviewStats.averageResponseTime, Clock, 'bg-purple-500')}
          </div>
              ...
              <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                <div className="flex items-center space-x-4">
                  ...
                  <div className="flex gap-2">
                    {['all', '5', '4', '3', 'hasImage'].map(rating => <Button key={rating} variant="ghost" size="sm" onClick={() => handleFilterChange(rating)} className={`px-3 py-1 rounded-full text-sm ${filterRating === rating ? 'bg-blue-500 text-white' : 'bg-white/10 text-white/80 hover:bg-white/20 border border-white/30'}`}>
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <select value={sortBy} onChange={e => handleSortChange(e.target.value)} className="bg-white/10 border border-white/30 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-white/50">
      ...
      {activeTab === 'feedback' && <div className="space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <select className="w-full bg-white/10 border border-white/30 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-white/50">
                      ...
                      <option value="1">AI智能染发自动调色宝机</option>
                      <option value="2">AI品牌染发膏管理系统</option>
                      <option value="3">AI客户配方管理系统</option>
                      <option value="4">AI美发连锁门店管理系统</option>
                      <option value="5">AI美发客户管理系统CRM</option>
                      <option value="6">AI染发色彩大师SaaS系统</option>
          ...
          <Card className="bg-white/10 backdrop-blur-md border-white/20">
            <CardContent className="p-8">
              <h2 className="text-2xl font-bold text-white mb-6">我的反馈</h2>
              <div className="space-y-4">
                {feedbackHistory.map(feedback => <div key={feedback.id} className="bg-white/10 rounded-xl p-4">
                    <div className="flex items-center justify-between mb-2">
                      <h3 className="text-white font-medium">{feedback.title}</h3>
                      <span className={`status-badge ${feedback.statusColor} px-2 py-1 rounded-full text-sm`}>
                        {feedback.statusText}
                      </span>
                    </div>
                    <p className="text-white/60 text-sm mb-2">{feedback.content}</p>
                    <div className="flex items-center justify-between">
                      <span className="text-white/40 text-xs">{feedback.date} 提交</span>
      ...
      {activeTab === 'stats' && <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {renderStatCard('平均评分', dataStats.averageRating, Star, 'bg-yellow-500')}
            {renderStatCard('总评价数', dataStats.totalReviews.toLocaleString(), Comments, 'bg-blue-500')}
            {renderStatCard('反馈数量', dataStats.totalFeedback, MessageSquare, 'bg-green-500')}
            {renderStatCard('解决率', `${dataStats.resolutionRate}%`, CheckCircle, 'bg-purple-500')}
          </div>

          {/* 趋势图表 */}
          <Card className="bg-white/10 backdrop-blur-md border-white/20">
            <CardContent className="p-8">
              <h2 className="text-2xl font-bold text-white mb-6">反馈趋势分析</h2>
              <div className="bg-white/10 rounded-lg p-6 h-64 flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-white/60">反馈趋势图表</p>
                  <p className="text-white/40 text-sm mt-2">显示最近6个月的反馈变化趋势</p>
                </div>
              </div>
            </CardContent>
          </Card>
